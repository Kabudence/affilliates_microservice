from rental.domain.entities.plan import Plan, PlanType  # Asegúrate de importar PlanType
...
class PlanCommandService:
    ...
    def create(self, name: str, description: str, duration: int, price: float, app_id: int, plan_type: PlanType, ids_modules: list[int]) -> Plan:
        if not name or not description or duration is None or price is None or app_id is None or plan_type is None:
    ...
    def update(self, plan_id: int, name: str, description: str, duration: int, price: float, app_id: int, plan_type: PlanType, ids_modules: list[int] = None) -> Plan:
        ...
        plan.price = price
        plan.app_id = app_id
        plan.plan_type = plan_type           # <--- NUEVO
